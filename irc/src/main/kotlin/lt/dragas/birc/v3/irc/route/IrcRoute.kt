 ...
 */
abstract class IrcRoute(open protected val type: Command, pattern: Pattern, callback: (Request) -> Response?) : Route<Request, Response>(pattern, callback)
{
    override fun canTrigger(request: Request): Boolean
    {
        return request.command == type.value
    }
}